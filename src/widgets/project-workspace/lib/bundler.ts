...
export interface BuildInput {
  ...
  // perRoute only: did the surgical router patch recognize a router? The caller
  // surfaces this (canvas frames can't boot their route if it stayed false).
  routerPatched: boolean;
}
...
export async function buildProjectFromFiles(
  ...
  const input = makeBuildInput(files, env, opts);
  ...
  if (workerBuildEnabled()) {
    try {
      ...
      return {
        code,
        dependencies: input.externalDepsMap,
        stats,
        routerPatched: input.routerPatched,
      };
    } catch (err) {
