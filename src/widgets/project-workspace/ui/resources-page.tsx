...
} from "@/features/google-calendar-integration";
import {
  MetaAdsLeadsModal,
  metaAdsIntegrationApi,
} from "@/features/meta-ads-integration";
import { googleAdsIntegrationApi } from "@/features/google-ads-integration";
...
const META_ADS_TYPE_VALUE = 1003;
const GOOGLE_ADS_TYPE_VALUE = 1004;
...
const normalizeApiResource = (item: any) => {
  const numericType =
    typeof item.resource_type === "number"
      ? item.resource_type
      : (RESOURCE_TYPE_STRING_TO_VALUE[String(item.type ?? "").toUpperCase()] ??
        null);
  ...
  const isGoogleCalendar =
    ...
    String(item.name ?? "")
      .trim()
      .toLowerCase() === "google calendar";
  const isMetaAds =
    numericType === META_ADS_TYPE_VALUE ||
    canonType === "METAADS" ||
    canonType === "META" ||
    String(item.provider ?? "").toLowerCase() === "meta-ads" ||
    String(item.name ?? "")
      .trim()
      .toLowerCase() === "meta ads";
  const isGoogleAds =
    numericType === GOOGLE_ADS_TYPE_VALUE ||
    canonType === "GOOGLEADS" ||
    String(item.provider ?? "").toLowerCase() === "google-ads" ||
    String(item.name ?? "")
      .trim()
      .toLowerCase() === "google ads";
...
const resourceCategories = [
  ...
  },
  {
    ...
    items: [
      {
        label: "Google Drive",
        typeValue: GOOGLE_DRIVE_TYPE_VALUE,
        icon: "google-drive",
      },
      {
        label: "Google Calendar",
        typeValue: GOOGLE_CALENDAR_TYPE_VALUE,
  ...
  },
  {
    id: "transcoder",
    label: "Transcoder",
    items: [{ label: "Transcoder", typeValue: 13, icon: "transcoder" }],
  },
  {
    id: "advertising",
    label: "Advertising",
    items: [
      { label: "Meta Ads", typeValue: META_ADS_TYPE_VALUE, icon: "meta-ads" },
      {
        label: "Google Ads",
        typeValue: GOOGLE_ADS_TYPE_VALUE,
        icon: "google-ads",
      },
    ],
  },
...
const ResourceIcon = ({ type }: { type: string }) => {
  const icons: Record<string, React.ReactNode> = {
    "google-drive": (
      <svg width="22" height="20" viewBox="0 0 87.3 78" aria-hidden="true">
        ...
        <path
          fill="#00AC47"
          d="M43.65 25 29.9 1.2c-1.35.8-2.5 1.9-3.3 3.3l-25.4 44A9.06 9.06 0 0 0 0 53h27.5L43.65 25Z"
        />
        <path
          fill="#EA4335"
          d="M73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75 7.65-13.25c.8-1.4 1.2-2.95 1.2-4.5H58.55l5.85 11.5L73.55 76.8Z"
        />
        <path
          fill="#00832D"
          d="M43.65 25 57.4 1.2c-1.35-.8-2.9-1.2-4.5-1.2H34.4c-1.6 0-3.15.45-4.5 1.2L43.65 25Z"
        />
        <path
          fill="#2684FC"
        ...
        <path
          fill="#FFBA00"
          d="M73.4 26.5 60.7 4.5c-.8-1.4-1.95-2.5-3.3-3.3L43.65 25l16.15 28h27.45c0-1.55-.4-3.1-1.2-4.5L73.4 26.5Z"
        />
      </svg>
    ),
    "google-calendar": (
      <svg width="22" height="22" viewBox="0 0 200 200" aria-hidden="true">
        ...
        <path
          fill="#4285f4"
          d="M71.6 124.3c-4-2.7-6.7-6.6-8.2-11.8l9-3.7c.8 3.2 2.3 5.6 4.4 7.4 2 1.7 4.5 2.6 7.4 2.6 3 0 5.5-.9 7.6-2.7 2.1-1.8 3.1-4.1 3.1-6.9 0-2.9-1.1-5.2-3.3-7-2.2-1.8-5-2.7-8.2-2.7h-5.2v-8.9h4.7c2.8 0 5.2-.8 7.1-2.3 2-1.5 2.9-3.6 2.9-6.3 0-2.4-.9-4.3-2.6-5.7-1.7-1.4-4-2.1-6.6-2.1-2.6 0-4.7.7-6.2 2.1-1.6 1.4-2.7 3.1-3.4 5.1l-8.9-3.7c1.1-3.2 3.2-6 6.2-8.5 3-2.5 6.9-3.7 11.6-3.7 3.5 0 6.6.7 9.4 2 2.7 1.4 4.9 3.2 6.4 5.6 1.6 2.4 2.3 5.1 2.3 8 0 3-.7 5.6-2.2 7.6-1.4 2.1-3.3 3.7-5.4 4.8v.5c2.8 1.2 5.2 3 7 5.5 1.8 2.5 2.8 5.5 2.8 9 0 3.5-.9 6.6-2.7 9.3-1.8 2.7-4.2 4.9-7.4 6.4-3.1 1.5-6.6 2.3-10.6 2.3-4.6.1-8.8-1.3-12.8-4z"
        />
        <path
          fill="#4285f4"
          d="M124.5 79.3l-9.9 7.2-5-7.6 17.8-12.8h6.8v60.4h-9.7z"
        />
      </svg>
    ),
    github: (
    ...
    gitlab: (
      ...
      >
        <path
          d="M16.7673 8.54208L16.7475 8.48981L14.842 3.32576C14.8032 3.22453 14.7346 3.13866 14.6459 3.08045C14.5571 3.02322 14.4536 2.99564 14.3493 3.00146C14.2451 3.00727 14.145 3.04618 14.0627 3.11295C13.9814 3.18172 13.9224 3.27473 13.8936 3.37943L12.607 7.46713H7.39713L6.11044 3.37943C6.08247 3.27416 6.02332 3.18069 5.94136 3.11225C5.85905 3.04549 5.75898 3.00657 5.65469 3.00076C5.55039 2.99495 5.44689 3.02252 5.35817 3.07975C5.26968 3.1382 5.20108 3.224 5.16202 3.32507L3.25293 8.4868L3.234 8.53908C2.67052 10.068 3.14902 11.7998 4.40763 12.7857L4.4142 12.791L4.43169 12.8039L7.33442 15.0611L8.77047 16.1897L9.64526 16.8756C9.74757 16.9563 9.87253 17 10.001 17C10.1295 17 10.2545 16.9563 10.3568 16.8756L11.2316 16.1897L12.6676 15.0611L15.5878 12.7902L15.5951 12.7842C16.8507 11.7981 17.3284 10.0694 16.7673 8.54208Z"
          fill="#E24329"
        />
        <path
          d="M16.7667 8.55169L16.747 8.5C15.8186 8.69571 14.9437 9.09949 14.1848 9.68251L10 12.9314C11.4251 14.0383 12.6657 15 12.6657 15L15.5858 12.7548L15.593 12.7489C16.8506 11.7738 17.329 10.063 16.7667 8.55169Z"
          fill="#FC6D26"
        />
        <path
          d="M7 15.0761L8.6156 16.196L9.59977 16.8766C9.71487 16.9566 9.85545 17 10 17C10.1446 17 10.2851 16.9566 10.4002 16.8766L11.3844 16.196L13 15.0761C13 15.0761 11.6026 14.1079 9.99925 13C8.99888 13.6913 7.99913 14.3833 7 15.0761Z"
          fill="#FCA326"
        />
        <path
          d="M5.8144 9.68551C5.0562 9.1012 4.18147 8.69635 3.25293 8.5L3.23399 8.55169C2.67053 10.0633 3.14901 11.7756 4.40758 12.7503L4.41416 12.7556L4.43164 12.7683L7.33427 15L10 12.9313L5.8144 9.68551Z"
          fill="#FC6D26"
        />
      </svg>
    ),
    bitbucket: (
      ...
      >
        <path
          d="M2.99 5.99a1.5 1.5 0 0 0-1.48 1.74l8.06 48.96a2.04 2.04 0 0 0 2 1.71h38.66a1.5 1.5 0 0 0 1.49-1.26l8.06-49.4a1.5 1.5 0 0 0-1.48-1.75L2.99 5.99ZM37.13 40.3H24.94l-3.3-17.25h18.4L37.13 40.3Z"
          fill="#2684FF"
        />
        <path
          d="M59.36 23.05H40.04l-3.24 17.25H24.94l-15.8 18.75c.5.43 1.13.67 1.79.68h38.67a1.5 1.5 0 0 0 1.49-1.26l8.28-35.42Z"
          fill="url(#bb-grad)"
        />
        <defs>
          <linearGradient
    ...
    superset: (
      ...
      >
        <path
          fill="#484848"
          d="M17.7115 6.167c-2.0149 0 -3.871 1.133575 -5.664775 3.114225C10.28405 7.269425 8.396825 6.167 6.2885 6.167 2.79745 6.167 0.25 8.658375 0.25 12.0124c0 3.354025 2.54745 5.814275 6.0385 5.814275 2.1457 0 3.8087 -1.0059 5.664775 -3.01145 1.7938 2.011775 3.61875 3.017675 5.758225 3.017675C21.20255 17.826675 23.75 15.375775 23.75 12.0124S21.20255 6.167 17.7115 6.167ZM6.3134 14.3076c-1.47925 0 -2.360575 -0.97475 -2.360575 -2.26405 0 -1.2893 0.881325 -2.292075 2.360575 -2.292075 1.2457 0 2.26405 1.002775 3.3665 2.35435 -1.04015 1.25815 -2.139475 2.201775 -3.3665 2.201775Zm11.292225 0c-1.227025 0 -2.26405 -0.97475 -3.3665 -2.26405 1.1336 -1.351575 2.10835 -2.292075 3.3665 -2.292075 1.47925 0 2.35125 1.01525 2.35125 2.292075s-0.872 2.26405 -2.35125 2.26405Z"
          strokeWidth="0.25"
        />
        <path
          fill="#20a7c9"
          d="m14.58165 16.99515 2.34815 -2.8028c-0.934275 -0.280275 -1.796925 -1.121125 -2.6907 -2.1644l-2.28585 2.78725c0.74835 0.86825 1.63685 1.605075 2.6284 2.17995Z"
          strokeWidth="0.25"
        />
        <path
          fill="#20a7c9"
          d="M12.04685 9.278c-0.743375 -0.8807 -1.62625 -1.634025 -2.61285 -2.229775l-2.351225 2.83395c0.893775 0.311425 1.6848 1.1149 2.525625 2.139475l0.093425 0.0654 2.345025 -2.80905Z"
          strokeWidth="0.25"
        />
      </svg>
    ),
    ...
    "meta-ads": <img src="/meta.svg" alt="Meta Ads" width={22} height={22} />,
    "google-ads": (
      <img src="/googleads.svg" alt="Google Ads" width={22} height={22} />
    ),
    postgresql: (
  ...
  return (
    <div
      className={cn(
        "border-border-subtle flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl border",
        !icon && "bg-bg-sidebar",
      )}
    >
      {icon || <Box className="text-text-muted h-5 w-5" />}
    </div>
  );
};

const buildMetaAdsRedirectUri = () =>
  // localePrefix is "never" (see i18n routing), so public URLs carry no locale
  // segment — the callback is served at /settings/meta-ads-callback.
  `${window.location.origin}/settings/meta-ads-callback`;

const metaAdsOAuthMessage = (reason?: string | null, detail?: string | null) => {
  if (reason === "facebook_access_denied") {
    return "You declined access. Allow page access to connect Meta.";
  }
  if (reason?.startsWith("facebook_")) {
    return `Facebook returned an error${detail ? `: ${detail}` : ""}. Try again.`;
  }
  if (reason === "invalid_state") {
    return "The Meta connection session expired. Start again.";
  }
  if (reason === "missing_code") {
    return "Meta sign-in did not complete. Try again.";
  }
  if (
    reason === "code_exchange_failed" ||
    reason === "long_lived_exchange_failed"
  ) {
    return "Could not finish connecting Meta. Try again later.";
  }
  if (reason === "token_store_failed") {
    return "Could not save the Meta connection. Try again.";
  }
  return detail || "Meta Ads connection failed";
};
...
export const ResourcesPage = ({ projectId }: { projectId: string }) => {
  ...
  const [editingResourceType, setEditingResourceType] = useState<number | null>(
    null,
  );
  ...
  const { data: metaAdsIntegrationData } = useQuery({
    queryKey: ["meta-ads-integration", projectId],
    queryFn: () => metaAdsIntegrationApi.getIntegration(),
    enabled: !!projectId && metaAdsStatus?.connected === true,
    retry: false,
    staleTime: 60 * 1000,
  });
  ...
  const { mutate: disconnectGithub, isPending: isDisconnectingGithub } =
    useMutation({
  ...
  const { mutate: connectGoogleDrive, isPending: isConnectingGoogleDrive } =
    useMutation({
      mutationFn: async (popup: Window | null) => {
        try {
          ...
          let apiKey =
            session.apiKey && session.apiKeyProjectId === projectId
              ? session.apiKey
              : null;
          if (!apiKey) {
          ...
          const url = await googleDriveIntegrationApi.getConnectUrl(
            apiKey,
            projectId,
          );
          if (popup && !popup.closed) popup.location.href = url;
  ...
  const {
    ...
    isPending: isConnectingGoogleCalendar,
  } = useMutation({
    mutationFn: async (popup: Window | null) => {
      try {
        ...
        const url = await googleCalendarIntegrationApi.getConnectUrl(
          apiKey,
          projectId,
        );
        if (popup && !popup.closed) popup.location.href = url;
  ...
  const { mutate: connectMetaAds, isPending: isConnectingMetaAds } =
    useMutation({
      mutationFn: async (popup: Window | null) => {
        try {
          const url = await metaAdsIntegrationApi.getConnectUrl(
  ...
  const { mutate: disconnectMetaAds, isPending: isDisconnectingMetaAds } =
    useMutation({
      mutationFn: async () => {
        const ids = (metaAdsIntegrationData?.integrations ?? [])
          .map((integration) => integration.resource_id)
          .filter(Boolean);

        if (ids.length === 0) {
          throw new Error("No Meta page subscriptions to disconnect");
        }

        await Promise.all(ids.map((id) => metaAdsIntegrationApi.disconnect(id)));
      },
      onSuccess: () => {
      ...
      onError: (err: any) => {
        toast.error(
          err?.response?.data?.data ||
            err?.message ||
            "Failed to disconnect Meta Ads",
        );
      },
    });

  const { mutate: connectGoogleAds, isPending: isConnectingGoogleAds } =
    useMutation({
      mutationFn: async (popup: Window | null) => {
        try {
          const session = useAuthStore.getState();
          ...
          const url = await googleAdsIntegrationApi.getConnectUrl(
            apiKey,
            projectId,
          );
          if (popup && !popup.closed) popup.location.href = url;
  ...
  const integrationScope = {
    ...
    environment_id: integrationEnvId,
  };
  ...
  const { mutate: disconnectGitlab, isPending: isDisconnectingGitlab } =
    useMutation({
  ...
  const { mutate: connectBitbucket, isPending: isConnectingBitbucket } =
    useMutation({
      mutationFn: async (popup: Window | null) => {
        try {
          const url =
            await bitbucketIntegrationApi.getConnectUrl(integrationScope);
          if (popup && !popup.closed) popup.location.href = url;
          else window.open(url, "_blank");
        } catch (err) {
          popup?.close();
  ...
  const { mutate: disconnectBitbucket, isPending: isDisconnectingBitbucket } =
    useMutation({
  ...
  const openOAuthPopup = () =>
    window.open(
      "about:blank",
      "_blank",
      centeredPopupFeatures(600, 720, "popup"),
    );

  // OAuth popups postMessage back here when the connection lands, so refresh
  // the matching integration card. Meta Lead Ads redirects back to this page
  // with facebook=success/error, then the popup forwards that result.
  useEffect(() => {
    ...
    const handleFacebookOAuthResult = (result: {
      success: boolean;
      reason?: string | null;
      detail?: string | null;
    }) => {
      setMetaAdsDialogOpen(true);
      if (result.success) {
        setIsMetaAdsConnected(true);
        queryClient.invalidateQueries({
          queryKey: ["meta-ads-status", projectId],
        });
        queryClient.invalidateQueries({
        ...
        toast.error(metaAdsOAuthMessage(result.reason, result.detail));
      }
    };
    ...
    if (facebookResult) {
      ...
      if (window.opener && window.opener !== window) {
        ...
        handleFacebookOAuthResult(result);
  ...
  const { data: resourceDetail } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(
        `/v2/company/project/resource/${editingResourceId}`,
        {
    ...
    mutationFn: async () => {
      ...
      if (isEditMode) {
        const settingsPayload = (() => {
          if (typeValue === 5)
            return {
              github: {
                ...(resourceDetail?.settings?.github || {}),
                ...settingsObject.github,
              },
            };
          if (typeValue === 8)
            return {
              gitlab: {
                ...(resourceDetail?.settings?.gitlab || {}),
                ...settingsObject.gitlab,
              },
            };
          return {
            ...(resourceDetail?.settings || {}),
      ...
      if (typeValue === 3) {
        ...
        return api.post("/v2/resource", {
          ...basePayload,
          default: false,
          settings: settingsObject.settings?.postgres
            ? { postgres: settingsObject.settings.postgres }
            : {},
        });
      }
  ...
  const handleSelectResource = (item: ResourceItem) => {
    ...
    if (item.typeValue === META_ADS_TYPE_VALUE) {
      setMetaAdsDialogOpen(true);
      connectMetaAds(openOAuthPopup());
      return;
    }

    if (item.typeValue === GOOGLE_ADS_TYPE_VALUE) {
      connectGoogleAds(openOAuthPopup());
      return;
    }
  ...
  const handleEditResource = (resource: any) => {
    ...
    if (numericType === META_ADS_TYPE_VALUE) {
      setMetaAdsDialogOpen(true);
      return;
    }
    const categoryItem = resourceCategories
      .flatMap((c) => c.items)
      .find((i) => i.typeValue === numericType);
  ...
  const ExtraField = ({
    ...
    <div className="animate-in fade-in space-y-1.5 duration-300">
      ...
      <Input
        ...
        onChange={(e) =>
          setExtraFields((prev) => ({ ...prev, [name]: e.target.value }))
        }
        disabled={disabled}
        className="bg-bg-sidebar border-border-subtle focus:ring-primary/20 focus:ring-1"
      />
    </div>
  );

  if (isListLoading && view === "grid")
    return <DataLoadingState message="Connecting to your resources..." />;
  ...
  if (view === "detail" && selectedResource) {
    ...
    return (
      <div className="animate-in fade-in slide-in-from-right-4 space-y-6 duration-300">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-text-main text-xl leading-tight font-bold">
                  {isEditMode ? "Edit" : "Connect"} {selectedResource.label}
                </h1>
        ...
        <div
          className={cn(
            "bg-bg-card border-border-subtle rounded-2xl border p-6 shadow-sm",
        ...
        >
          <div className="space-y-5">
            ...
            {(rType === 5 || rType === 8) && !isEditMode && (
              <div className="bg-bg-sidebar border-border-subtle text-text-muted rounded-xl border p-4 text-center text-sm">
                Click "Authenticate with {rType === 5 ? "GitHub" : "GitLab"}" on
                the resources grid to connect via OAuth.
              </div>
            )}
            ...
            {(rType === 11 || rType === 12) && isEditMode && (
              <>
                <ExtraField
                  label="URL"
                  name={`settings.${rType === 11 ? "superset" : "metabase"}.url`}
                  placeholder="URL"
                />
                <ExtraField
                  label="Username"
                  name={`settings.${rType === 11 ? "superset" : "metabase"}.username`}
                  placeholder="Username"
                />
                <ExtraField
                  label="Password"
                  name={`settings.${rType === 11 ? "superset" : "metabase"}.password`}
                  placeholder="Password"
                  type="password"
                />
              </>
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <Button
                disabled={
                  !formData.name ||
                  !formData.type ||
                  (!isEditMode && !formData.environment) ||
                  isSaving
                }
                onClick={() => handleSave()}
        ...
        {rType === 13 && pipelinesData?.length > 0 && (
          <div className="bg-bg-card border-border-subtle mt-6 max-w-lg overflow-hidden rounded-2xl border shadow-sm">
            <div className="bg-bg-sidebar border-border-subtle border-b px-4 py-3">
              <h3 className="text-text-main flex items-center gap-2 text-xs font-bold">
                <RefreshCw size={12} />
                Transcoder Pipelines
              </h3>
            </div>
            <Table>
              <TableHeader>
                <TableRow className="bg-bg-sidebar/50 border-b-border-subtle">
                  <TableHead className="text-text-muted w-12 text-center text-[10px] font-bold uppercase">
                    №
                  </TableHead>
                  <TableHead className="text-text-muted text-[10px] font-bold uppercase">
                    Status
                  </TableHead>
                  <TableHead className="text-text-muted text-[10px] font-bold uppercase">
                    Video Key
                  </TableHead>
                  <TableHead className="text-text-muted text-right text-[10px] font-bold uppercase">
                    Size
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {pipelinesData.map((item: any, idx: number) => {
                  ...
                  return (
                    <TableRow
                      key={item.id}
                      className="border-b-border-subtle/40 hover:bg-hover-bg/30 last:border-0"
                    >
                      ...
                      <TableCell className="text-text-main max-w-[140px] truncate font-mono text-[11px]">
                        {item.output_key}
                      </TableCell>
                      <TableCell className="text-text-muted text-right text-[11px]">
  ...
  const isGithubExpired =
    githubStatus?.connected === false &&
    githubStatus?.reason === "token_expired";

  const isGitlabConnected = gitlabStatus?.connected === true;
  const isGitlabExpired =
    gitlabStatus?.connected === false &&
    gitlabStatus?.reason === "token_expired";

  const isBitbucketConnected = bitbucketStatus?.connected === true;
  const isBitbucketExpired =
    bitbucketStatus?.connected === false &&
    bitbucketStatus?.reason === "token_expired";

  const metaAdsIntegrations = metaAdsIntegrationData?.integrations ?? [];
  const isMetaAdsExpired = metaAdsStatus?.connected && !metaAdsStatus.active;
  const hasMetaAdsConnection =
    isMetaAdsConnected ||
    metaAdsStatus?.connected === true ||
    metaAdsIntegrations.length > 0;

  const filteredConnectedResources = resourcesList.filter((resource: any) => {
    const categoryItem = resourceCategories
      .flatMap((c) => c.items)
      .find((i) => i.typeValue === resource.resource_type);
    const categoryId = resourceCategories.find((c) =>
      c.items.includes(categoryItem as any),
    )?.id;
  ...
  const connectedTypeValues = new Set(
    resourcesList.map((r: any) => r.resource_type),
  );
  ...
  if (isGoogleCalendarConnected)
    connectedTypeValues.add(GOOGLE_CALENDAR_TYPE_VALUE);
  if (hasMetaAdsConnection) connectedTypeValues.add(META_ADS_TYPE_VALUE);
  if (isGoogleAdsConnected) connectedTypeValues.add(GOOGLE_ADS_TYPE_VALUE);

  const availableResources = resourceCategories.flatMap((c) =>
  ...
  const filteredAvailableResources = availableResources.filter((item) => {
    ...
    const matchesSearch = item.label
      ...
      .includes(searchQuery.toLowerCase());
    const matchesCategory =
      categoryFilter === "All Categories" || item.categoryId === categoryFilter;
    return matchesSearch && matchesCategory;
  });
  ...
  const githubMatchesSearch =
    "github".includes(searchQuery.toLowerCase()) || searchQuery === "";
  const githubMatchesCategory =
    categoryFilter === "All Categories" || categoryFilter === "source_control";
  const gitlabMatchesSearch =
    "gitlab".includes(searchQuery.toLowerCase()) || searchQuery === "";
  const gitlabMatchesCategory =
    categoryFilter === "All Categories" || categoryFilter === "source_control";
  const bitbucketMatchesSearch =
    "bitbucket".includes(searchQuery.toLowerCase()) || searchQuery === "";
  const bitbucketMatchesCategory =
    categoryFilter === "All Categories" || categoryFilter === "source_control";
  const googleDriveMatchesSearch =
    "google drive".includes(searchQuery.toLowerCase()) || searchQuery === "";
  const googleDriveMatchesCategory =
    categoryFilter === "All Categories" || categoryFilter === "productivity";
  const googleCalendarMatchesSearch =
    "google calendar".includes(searchQuery.toLowerCase()) || searchQuery === "";
  const googleCalendarMatchesCategory =
    categoryFilter === "All Categories" || categoryFilter === "productivity";
  const metaAdsMatchesSearch =
    "meta ads".includes(searchQuery.toLowerCase()) || searchQuery === "";
  const metaAdsMatchesCategory =
    categoryFilter === "All Categories" || categoryFilter === "advertising";
  const googleAdsMatchesSearch =
    "google ads".includes(searchQuery.toLowerCase()) || searchQuery === "";
  const googleAdsMatchesCategory =
    categoryFilter === "All Categories" || categoryFilter === "advertising";

  // Once the refetched resource list contains the Drive/Calendar/Ads resource it
  ...
  const apiHasGoogleDrive = resourcesList.some(
    (r: any) => r.resource_type === GOOGLE_DRIVE_TYPE_VALUE,
  );
  const apiHasGoogleCalendar = resourcesList.some(
    (r: any) => r.resource_type === GOOGLE_CALENDAR_TYPE_VALUE,
  );
  const apiHasMetaAds = resourcesList.some(
    (r: any) => r.resource_type === META_ADS_TYPE_VALUE,
  );
  const apiHasGoogleAds = resourcesList.some(
    (r: any) => r.resource_type === GOOGLE_ADS_TYPE_VALUE,
  );

  return (
    <div className="animate-in fade-in @container flex h-full flex-col space-y-6 duration-500">
      <div>
        <h1 className="text-text-main text-2xl font-bold tracking-tight">
          Integrations
        </h1>
        <p className="text-text-muted mt-1 text-sm">
          Connect third-party services and extend your application
        </p>
      </div>

      <div className="border-border-subtle bg-bg-main sticky top-0 z-10 flex flex-wrap items-center gap-3 border-b py-3">
        <div className="bg-bg-sidebar border-border-subtle flex min-w-[160px] flex-1 items-center rounded-xl border px-3 py-2">
          <Search className="text-text-muted mr-2 h-4 w-4 shrink-0" />
          <input
            className="placeholder:text-text-muted text-text-main w-full border-none bg-transparent text-sm outline-none"
        ...
        <Select value={categoryFilter} onValueChange={setCategoryFilter}>
          <SelectTrigger className="bg-bg-sidebar border-border-subtle h-10 w-full rounded-xl @[480px]:w-45">
      ...
      <div className="no-scrollbar flex-1 overflow-y-auto pb-10">
        ...
        {showConnectedSection && (
          <div className="mb-8">
            <div className="text-text-muted mb-3 text-xs font-semibold tracking-[0.04em] uppercase">
              Connected
            </div>
            <div className="grid grid-cols-1 gap-4 @[480px]:grid-cols-2 @[720px]:grid-cols-3 @[1024px]:grid-cols-4">
              ...
              {isGoogleDriveConnected &&
                !apiHasGoogleDrive &&
                googleDriveMatchesSearch &&
                googleDriveMatchesCategory && (
                  <div
              ...
              {isGoogleCalendarConnected &&
                !apiHasGoogleCalendar &&
                googleCalendarMatchesSearch &&
                googleCalendarMatchesCategory && (
                  <div
                    className="bg-bg-card border-border-subtle flex flex-col gap-3 rounded-xl border p-4 shadow-sm"
                    style={{
                      borderLeftWidth: "3px",
              ...
              {hasMetaAdsConnection &&
                !apiHasMetaAds &&
                metaAdsMatchesSearch &&
                metaAdsMatchesCategory && (
                  <div
                    className="bg-bg-card border-border-subtle flex flex-col gap-3 rounded-xl border p-4 shadow-sm"
                    style={{
                      borderLeftWidth: "3px",
                      borderLeftColor: isMetaAdsExpired
                        ? "var(--destructive, #ef4444)"
                        : "var(--green, #22c55e)",
                    }}
                  >
                    <div className="flex flex-wrap items-center gap-3">
                      <ResourceIcon type="meta-ads" />
                      <div className="min-w-0 flex-1">
                        <div className="text-text-main truncate text-sm font-bold">
                          Meta Ads
                        </div>
                      ...
                      <span
                        className={cn(
                          "ml-auto shrink-0 rounded-md border px-2 py-0.5 text-[10px] font-bold tracking-wider uppercase",
                          isMetaAdsExpired
                            ? "bg-destructive/10 text-destructive border-destructive/20"
                            : "border-green-500/20 bg-green-500/10 text-green-500",
                        )}
                      >
                        {isMetaAdsExpired ? "Expired" : "Connected"}
                      </span>
                    </div>
                    <div className="text-text-muted text-xs leading-relaxed">
                      {isMetaAdsExpired
                        ? metaAdsStatus?.reason ||
                          "Reconnect Meta to restore lead access."
                        : `${metaAdsIntegrations.length} page${
                            metaAdsIntegrations.length === 1 ? "" : "s"
                          } subscribed for lead capture.`}
                    </div>
                    <div className="mt-auto flex flex-wrap gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        className="border-border-subtle bg-bg-main text-primary hover:bg-primary/5 min-w-27.5 flex-1 justify-center gap-2 rounded-lg font-semibold"
                        onClick={() => setMetaAdsDialogOpen(true)}
                      >
                        <SlidersHorizontal size={14} />
                        Manage leads
                      </Button>
                      <Button
                        ...
                        size="sm"
                        className="border-border-subtle bg-bg-main text-text-muted hover:bg-primary/5 hover:text-primary min-w-27.5 flex-1 justify-center gap-2 rounded-lg font-semibold"
                        onClick={() => connectMetaAds(openOAuthPopup())}
                        disabled={isConnectingMetaAds}
                      >
                        {isConnectingMetaAds ? (
                          <Loader2 size={14} className="animate-spin" />
                        ) : (
                        ...
                        {isConnectingMetaAds ? "Connecting…" : "Reconnect"}
                      </Button>
                      {metaAdsIntegrations.length > 0 && (
                        <Button
                          variant="outline"
                          size="sm"
                          className="text-destructive hover:bg-destructive/5 hover:text-destructive border-border-subtle bg-bg-main min-w-27.5 flex-1 justify-center gap-2 rounded-lg font-semibold"
                          onClick={() => {
                            const ok = window.confirm(
                              "Leads from all subscribed Meta pages will stop being received. Disconnect Meta Ads?",
                            );
                            if (ok) disconnectMetaAds();
                          }}
                          disabled={isDisconnectingMetaAds}
                        >
                          {isDisconnectingMetaAds ? (
                            <Loader2 size={14} className="animate-spin" />
                          ) : (
                            <Trash2 size={14} />
                          )}
              ...
              {isGoogleAdsConnected &&
                !apiHasGoogleAds &&
                googleAdsMatchesSearch &&
                googleAdsMatchesCategory && (
                  <div
                    className="bg-bg-card border-border-subtle flex flex-col gap-3 rounded-xl border p-4 shadow-sm"
                    style={{
                      borderLeftWidth: "3px",
                  ...
                  >
                    ...
                    <div className="text-text-muted text-xs leading-relaxed">
                      Google Ads authorization completed successfully.
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      className="border-border-subtle bg-bg-main text-text-muted hover:bg-primary/5 hover:text-primary mt-auto w-full justify-center gap-2 rounded-lg font-semibold"
                      onClick={() => connectGoogleAds(openOAuthPopup())}
                    ...
                    >
                      ...
                      {isConnectingGoogleAds ? "Connecting…" : "Reconnect"}
                    </Button>
                  </div>
                )}

              {/* GitHub card (new API) */}
              {(isGithubConnected || isGithubExpired) &&
                githubMatchesSearch &&
                githubMatchesCategory && (
                  <div
                    className="bg-bg-card border-border-subtle flex flex-col gap-3 rounded-xl border p-4 shadow-sm"
                    style={{
                      borderLeftWidth: "3px",
                      borderLeftColor: isGithubConnected
                        ? "var(--green, #22c55e)"
                        : "var(--destructive, #ef4444)",
                    }}
                  >
                    ...
                    {isGithubConnected && githubStatus.user?.avatar_url && (
                      <div className="bg-bg-sidebar border-border-subtle flex items-center gap-2 rounded-lg border px-2 py-1.5">
                        <img
                    ...
                    <div className="mt-auto flex flex-wrap gap-2">
                      {isGithubConnected && (
                        <Button
                          ...
                          onClick={() =>
                            githubIntegration &&
                            disconnectGithub(githubIntegration.id)
                          }
              ...
              {/* GitLab card (new API) */}
              {(isGitlabConnected || isGitlabExpired) &&
                gitlabMatchesSearch &&
                gitlabMatchesCategory && (
                  <div
                    className="bg-bg-card border-border-subtle flex flex-col gap-3 rounded-xl border p-4 shadow-sm"
                    style={{
                      borderLeftWidth: "3px",
                      borderLeftColor: isGitlabConnected
                        ? "var(--green, #22c55e)"
                        : "var(--destructive, #ef4444)",
                    }}
                  >
                    ...
                    {isGitlabConnected && gitlabStatus.user?.avatar_url && (
                      <div className="bg-bg-sidebar border-border-subtle flex items-center gap-2 rounded-lg border px-2 py-1.5">
                        <img
                          ...
                          alt={gitlabStatus.user.username}
                          className="h-5 w-5 shrink-0 rounded-full"
                        />
                        <span className="text-text-main truncate text-xs font-medium">
                          @{gitlabStatus.user.username}
                    ...
                    <div className="mt-auto flex flex-wrap gap-2">
                      {isGitlabConnected && (
                        <Button
                          ...
                          onClick={() =>
                            gitlabIntegration &&
                            disconnectGitlab(gitlabIntegration.id)
                          }
              ...
              {(isBitbucketConnected || isBitbucketExpired) &&
                bitbucketMatchesSearch &&
                bitbucketMatchesCategory && (
                  <div
                    className="bg-bg-card border-border-subtle flex flex-col gap-3 rounded-xl border p-4 shadow-sm"
                    style={{
                      borderLeftWidth: "3px",
                      borderLeftColor: isBitbucketConnected
                        ? "var(--green, #22c55e)"
                        : "var(--destructive, #ef4444)",
                    }}
                  >
                    ...
                    {isBitbucketConnected &&
                      bitbucketStatus.user?.avatar_url && (
                        <div className="bg-bg-sidebar border-border-subtle flex items-center gap-2 rounded-lg border px-2 py-1.5">
                          <img
                            src={bitbucketStatus.user.avatar_url}
                            alt={bitbucketStatus.user.username}
                            className="h-5 w-5 shrink-0 rounded-full"
                          />
                          <span className="text-text-main truncate text-xs font-medium">
                            @{bitbucketStatus.user.username}
                      ...
                      {isBitbucketConnected && (
                        <Button
                          ...
                          onClick={() =>
                            ...
                            disconnectBitbucket(bitbucketIntegration.id)
                          }
                          disabled={
                            isDisconnectingBitbucket || !bitbucketIntegration
                          }
                        >
                          {isDisconnectingBitbucket ? (
                            <Loader2 size={14} className="animate-spin" />
                          ) : (
                            <Trash2 size={14} />
                          )}
              ...
              {filteredConnectedResources.map((resource: any) => {
                const typeInfo = resourceTypes.find(
                ...
                return (
                  <div
                    ...
                    className="bg-bg-card border-border-subtle group flex cursor-pointer flex-col gap-3 rounded-xl border p-4 shadow-sm transition-all hover:shadow-md"
                    style={{
                      borderLeftWidth: "3px",
                      borderLeftColor: "var(--green, #22c55e)",
                    }}
                  >
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-text-main group-hover:text-primary truncate text-sm font-bold transition-colors">
                          {resource.name}
                        </div>
                        <div className="text-text-muted text-[11px] font-medium">
                          {typeInfo?.label ?? categoryItem?.label}
                        </div>
                      </div>
                      <span className="ml-auto shrink-0 rounded-md border border-green-500/20 bg-green-500/10 px-2 py-0.5 text-[10px] font-bold tracking-wider text-green-500 uppercase">
                    ...
                    <div className="text-text-muted text-xs leading-relaxed">
                      Integration for {typeInfo?.label ?? categoryItem?.label} —{" "}
                      {resource.name}
                    </div>
                  </div>
                );
              ...
              {filteredAvailableResources.map((item) => (
                ...
                >
                  ...
                  <div className="text-text-muted flex-1 text-xs leading-relaxed">
                    Connect {item.label} to configure your infrastructure
                    integration.
                  </div>
                  {(() => {
                    const isConnecting =
                      ...
                      (item.typeValue === GOOGLE_DRIVE_TYPE_VALUE &&
                        isConnectingGoogleDrive) ||
                      (item.typeValue === GOOGLE_CALENDAR_TYPE_VALUE &&
                        isConnectingGoogleCalendar) ||
                      (item.typeValue === META_ADS_TYPE_VALUE &&
                        isConnectingMetaAds) ||
                      (item.typeValue === GOOGLE_ADS_TYPE_VALUE &&
                        isConnectingGoogleAds);
                    return (
                      <Button
                        variant="outline"
                        size="sm"
                        className="text-text-main hover:bg-primary/5 hover:text-primary border-border-subtle bg-bg-main w-full justify-center gap-2 rounded-lg font-semibold"
      ...
      {metaAdsDialogOpen && (
        <MetaAdsLeadsModal
          open={metaAdsDialogOpen}
          onOpenChange={setMetaAdsDialogOpen}
          onConnect={() => connectMetaAds(openOAuthPopup())}
          isConnecting={isConnectingMetaAds}
          projectId={projectId}
        />
      )}
    </div>
