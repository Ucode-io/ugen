...
export const ProjectCodeViewer = ({
          ...
          return (
            <div key={node.path} className="select-none">
              <div
                ...
                style={{ paddingLeft: `${(isFolder ? level : level + 1) * 12 + 8}px` }}
              >
                {isFolder ? (
                  <>
                    <ChevronRight
  ...
  return (
    <div className="bg-bg-main flex h-full w-full flex-1 overflow-hidden text-[13px]">
      {/* Sidebar */}
      <div className="border-border-subtle bg-bg-card flex w-72 shrink-0 flex-col border-r">
        <div className="bg-bg-main border-border-subtle flex shrink-0 border-b p-1">
          <button
            onClick={() => setSidebarMode("explorer")}
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-sm py-1.5 transition-colors ${sidebarMode === "explorer" ? "bg-primary/10 text-primary font-medium" : "text-text-muted hover:text-text-main"}`}
          >
            <Files size={14} /> Explorer
          </button>
          <button
            onClick={() => setSidebarMode("search")}
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-sm py-1.5 transition-colors ${sidebarMode === "search" ? "bg-primary/10 text-primary font-medium" : "text-text-muted hover:text-text-main"}`}
          >
            ...
            <div className="border-border-subtle shrink-0 border-b p-3">
              ...
              {searchQuery && (
                <div className="text-text-muted mt-2 text-xs">
                  Found {globalSearchResults.length}{" "}
                  {globalSearchResults.length === 1 ? "match" : "matches"}
                </div>
              )}
            </div>
      ...
      <div className="bg-bg-main flex h-full min-w-0 flex-1 flex-col">
          ...
          <div className="bg-bg-card border-border-subtle custom-scrollbar flex shrink-0 overflow-x-auto border-b">
            {openedFiles.map((path) => (
              <div
                ...
                className={`group border-border-subtle flex max-w-[200px] min-w-[120px] cursor-pointer items-center gap-2 border-r px-4 py-2 transition-colors select-none ${activeFile === path ? "bg-bg-main text-primary border-t-primary border-t-2" : "text-text-muted hover:bg-hover-bg border-t-2 border-t-transparent"}`}
              >
                {getFileIcon(path)}
                ...
                <button
                  ...
                  className={`rounded-sm p-0.5 hover:bg-black/10 dark:hover:bg-white/10 ${activeFile === path ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                >
                  <X size={14} />
                </button>
              </div>
        ...
        {/* Action Bar */}
        {activeFile && (
          <div className="bg-bg-main border-border-subtle flex shrink-0 items-center justify-end border-b px-4 py-1.5">
            <span className="text-text-muted mr-auto font-mono text-xs opacity-70">
              {activeFile}
            </span>
            <button
              onClick={handleSave}
