...
const DEVICES: { id: DeviceId; icon: ReactNode }[] = [
...
export const TemplateDetailPage = ({ id }: TemplateDetailPageProps) => {
  const t = useTranslations('widgets.templateDetail')
  ...
  const { data: template, isLoading } = useQuery({
    queryKey: ['ugen-template-public', id],
    queryFn: () => fetchPublicTemplateDetail(id),
  ...
  if (isLoading) {
    return (
      <div className="bg-bg-main flex min-h-[60vh] items-center justify-center">
  ...
  if (!template) {
    return (
      <div className="bg-bg-main flex min-h-[60vh] flex-col items-center justify-center gap-4">
  ...
  const previewSection = (
    <div
      className={cn(
        ...
        isMaximized ? 'bg-bg-main fixed inset-0 z-[150]' : 'mb-3',
      )}
      style={isMaximized ? undefined : { height: 600 }}
    >
      <div
        className="border-border-subtle bg-bg-card relative flex h-full flex-shrink-0 flex-col overflow-hidden border shadow-md transition-all duration-300"
        style={{
          width: iframeWidth,
          maxWidth: '100%',
        ...
        {demoUrl && (
          <div className="border-border-subtle bg-bg-card flex h-10 shrink-0 items-center justify-end gap-1 border-b px-2">
            ...
            <button
              ...
              title={isMaximized ? t('exitFullscreen') : t('fullscreen')}
              className="text-text-muted hover:text-text-main hover:bg-hover-bg flex h-7 w-7 items-center justify-center rounded-lg transition-colors"
            >
  ...
  return (
    <div className="bg-bg-main flex min-h-screen flex-col">
      <div className="mx-auto w-full max-w-[1120px] flex-1 px-6 pt-6 pb-20">
        <div className="mb-6 flex items-center gap-4">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button
              ...
              className="inline-flex h-9 items-center gap-2 rounded-lg bg-blue-500 px-4 text-sm font-semibold text-white transition-colors hover:bg-blue-600"
            >
              {t('useTemplate')}
            </button>
          </div>
            ...
            <div className="flex flex-wrap gap-3">
              {images.map((img, i) => (
                ...
                >
                  ...
                  <img
                    src={img}
                    alt={t('screenshot', { index: String(i + 1) })}
