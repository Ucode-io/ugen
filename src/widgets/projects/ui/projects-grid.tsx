import { Plus, Star, Link as LinkIcon, MoreHorizontal } from "lucide-react"
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      ...
      {projects.map(project => (
        <Link href={`/projects/${project.id}`} key={project.id} className="group cursor-pointer block">
          <div className="relative aspect-[16/9] w-full overflow-hidden rounded-2xl border border-border-subtle bg-bg-sidebar transition-all">
            {project.image ? (
            ...
            <div className="absolute right-3 top-3 opacity-0 transition-opacity group-hover:opacity-100">
              <button className="flex h-8 w-8 items-center justify-center rounded-lg bg-bg-main/60 text-text-main hover:bg-bg-main/80 backdrop-blur-sm transition-colors">
                <Star size={18} />
              </button>
            </div>
          </div>
          ...
          <div className="mt-3 flex items-start gap-3">
            ...
            <div className="flex-1 overflow-hidden">
              <h3 className="truncate text-[15px] font-medium text-text-main group-hover:text-text-main">{project.name}</h3>
              <p className="truncate text-[13px] text-text-muted mt-0.5">{project.editedAt}</p>
            </div>

            <div className="hidden shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100 sm:flex mt-1">
              <button className="flex h-7 w-7 items-center justify-center rounded-md text-text-main hover:bg-hover-bg transition-colors">
                <LinkIcon size={16} />
              </button>
              <button className="flex h-7 w-7 items-center justify-center rounded-md text-text-main hover:bg-hover-bg transition-colors">
