...
export const FolderCard = ({ folder, readOnly = false }: { folder: { id: string, name: string }; readOnly?: boolean }) => {
  return (
    <div className="group relative flex w-[200px] h-[58px] items-center rounded-2xl border border-border-subtle bg-bg-sidebar px-4 hover:border-text-muted transition-colors cursor-pointer shrink-0">
      <Link href={`/projects?folder_id=${folder.id}`} className={`flex items-center gap-3 flex-1 h-full min-w-0 ${readOnly ? "" : "pr-6"}`}>
