...
export const googleDriveIntegrationApi = {
  getConnectUrl: async (
    apiKey: string,
    mcpProjectId: string,
  ): Promise<string> => {
    const { data } = await api.get("/v1/google-drive/connect", {
      params: {
    ...
    if (!url) {
      throw new Error("Google Drive connect endpoint returned no OAuth URL");
    }
    return url;
  },
};
